 ...
 * otp_dec pcipher and a key. It then writes back the deciphered text to the otp_dec process that is connected
 * via the same communication socket. It is nearly identical to otp_enc_d
 * */
...
// Function decrypt the characters
void decryption(char message[], char key[], int length)
{
    int n, text, keyNumber, decryptionNumber;
    ...
    for (n = 0; n < (length + 1); n++)
    {
        ...
        decryptionNumber = (text - keyNumber) % 27;
        
        // If the decryption number is less than 0 add 27 to get the correct integer
        if (decryptionNumber < 0)
        {
            decryptionNumber += 27;
        }
        // Decrypt the plaintext to original char
        message[n] = intToChar(decryptionNumber);
    }
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (pid == 0)
        {
            ...
            if (textSize < 0)
            {
                fprintf(stderr, "Error port: %d failed to read file\n", port);
                exit(1);
            }
           ...
           // Encrypt the plain text
            decryption(buffer, keyBuffer, strlen(buffer));

            
            // Send deciphered text back to client
